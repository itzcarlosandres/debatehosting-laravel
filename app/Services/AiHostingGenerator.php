...
PROMPT;
    ...
    /**
     * Obtiene la configuración de Gemini (API Key y modelo) desde Settings o config/env
     */
    ...
    /**
     * Realiza un ping/test de conexión con la API de Google Gemini
     */
    public function testGeminiConnection(?string $apiKey = null, ?string $model = null): array
    {
        $config = $this->getGeminiConfig();
        $key = ! empty($apiKey) ? $apiKey : $config['api_key'];
        $mdl = ! empty($model) ? $model : $config['model'];

        if (empty($key)) {
            return [
                'success' => false,
                'message' => 'No se ha configurado ninguna API Key para Gemini.',
            ];
        }
        ...
        try {
            ...
            if ($response->successful()) {
                return [
                    'success' => true,
                    'message' => "Conexión exitosa con Google Gemini ({$mdl}).",
                    'data' => $response->json(),
                ];
            }

            $errorMsg = $response->json('error.message') ?? 'Error en la llamada a la API de Gemini.';

            return [
                'success' => false,
                'message' => "Error de API ({$response->status()}): {$errorMsg}",
            ];
        } catch (\Throwable $e) {
            return [
                'success' => false,
